...
No much documentation so far. I hope it will change in the next days ;)
...
- Returns all namespace functions:

	functions=namespace.functions # Note that the related XML file is now parsed (lazily) and not at object creation


- Returns only some selected functions:

	functions=namespace.functions ["function1",...]
...
- Returns other classes also included in the same file (parses file myIncludeFile_8h.xml):

	other_classes = file.classes
...
- Returns Function properties:
	...
	puts myMethod.params.join(",")	# > const String &, const Any &
