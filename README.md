...
Using Doxygen allows us to parse even a set of non-compilable include files. This is very useful in case you need to parse only a subset of a big library which will not normally compile because of being incomplete or needing further build configuration (e.g Makefiles, VS, SCons, etc).
...
Do not forget to set the EXTRACT_ALL = true flag in the Doxyfile and allow generation of XMLs
...
- Returns all (direct) namespace members:
	...
	my_typedefs = namespace.typedefs


- All of the above can also make optional use of arrays to select only the desired results

	functions=namespace.functions(["function1","function2","etc.."])
...
- Returns all (direct) class members:
	...
	my_class_typedefs = clazz.typedefs


- All of the above can also make optional use of filter arrays and access modifiers (public by default)
...
- For Methods
	...
	puts myMethod.name 		# > getProp
	
	puts myMethod.getter_for	# > prop
	
	puts myMethod.name 		# > set/isProp
